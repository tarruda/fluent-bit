                                    ...
                                    struct flb_config *config)
{
    ...
    /* Allocate context */
    lf = flb_calloc(1, sizeof(struct lua_input));
    if (!lf) {
        flb_errno();
        return NULL;
    }
    ret = flb_input_config_map_set(ins, (void*)lf);
    if (ret < 0) {
        flb_errno();
    ...
    /* Config: script */
    script = flb_input_get_property("script", ins);
    if (!script) {
    ...
    tmp = flb_input_get_property("type_array_key", ins);
    if (tmp) {
        split = flb_utils_split(tmp, ' ', L2C_TYPES_NUM_MAX);
        mk_list_foreach_safe(head, tmp_list, split) {
            l2c = flb_malloc(sizeof(struct l2c_type));

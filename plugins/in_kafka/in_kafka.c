...
static int flb_input_thread_init(struct flb_input_thread *it, flb_input_thread_cb callback, void *data)
{
    flb_pipefd_t fd[2];
    int result;
...
static int flb_input_thread_collect(struct flb_input_instance *ins,
                             ...
                             void *in_context)
{
    ...
    flb_input_log_append(ins, NULL, 0, it->buf, chunks_len);
    memmove(it->buf, it->buf + chunks_len, remaining_bytes);
    it->bufpos = remaining_bytes;
    return 0;
}

static void flb_input_thread_exit(void *in_context, struct flb_input_instance *ins)
{
...
static int flb_input_thread_destroy(struct flb_input_thread *it, struct flb_input_instance *ins)
{
    int ret;
...
static bool flb_input_thread_exited(struct flb_input_thread *it)
{
    bool ret;
    pthread_mutex_lock(&it->mutex);
    ret = it->exit;
